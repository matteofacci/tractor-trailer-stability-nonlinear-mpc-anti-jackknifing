...
for i = 1:size(x_opt,2)
    ...
    legend([tractorTrailerLegendLabels(plotArrow,'Tractor','Trailer'),"Reference trajectory"],'Interpreter','latex','Location','northwest')
    ...
    if i < size(x_opt,2) % plot prediction
        ...
        switch model
            ...
            case "pointp_rwdtractortrailer"
                ...
                xR = x_pred(N,1,i) - L*cos(x_pred(N,3,i)) - d*cos(x_pred(N,3,i)+x_pred(N,6,i));
                yR = x_pred(N,2,i) - L*sin(x_pred(N,3,i)) - d*sin(x_pred(N,3,i)+x_pred(N,6,i));
                theta = x_pred(N,3,i);
                psi = x_pred(N,4,i);
                psi2 = x_pred(N,5,i);
                phi = x_pred(N,6,i);
        ...
        plotArrow = 0;
        plot_tractor(xR,xF,yR,yF,hitchJoint,theta,phi,tractorWidth,L,tractorBodyLenght,tractorRGB,wheelWidth,wheelDiam,wheelRGB,plotArrow,0.1)
        plot_trailer(xTrailer,yTrailer,hitchJoint,theta,psi,trailerWidth,L2,trailerBodyLenght,trailerRGB,wheelWidth,wheelDiam,wheelRGB,hitchJoint2,plotArrow,0.1)
        plot_trailer(xTrailer2,yTrailer2,hitchJoint2,theta+psi,psi2,trailerWidth,L4,trailerBodyLenght,trailerRGB,wheelWidth,wheelDiam,wheelRGB,[],plotArrow,0.1)
...
videoName = sprintf('tractor_trailer_%s', datestr(now,'dd-mm-yyyy HH-MM'));
